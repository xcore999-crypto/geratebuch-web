...
import { legalNav, contact, externalLinks } from "@/lib/nav";
    ...
    <footer className="border-t border-white/10 bg-navy-950 text-slate-300">
      <div className="mx-auto max-w-7xl px-6 py-20 lg:px-8">
        <div className="grid gap-12 lg:grid-cols-[1.4fr_1fr_1fr_1fr]">
          <div>
            <Logo variant="dark" />
            <p className="mt-6 max-w-xs text-base leading-relaxed text-slate-400">
              Die digitale Geräteakte und Serviceplattform für Kosmetik- und
              Lasergeräte in Deutschland.
            </p>
            <p className="mt-5 inline-flex rounded-full border border-white/10 px-3 py-1.5 text-[0.68rem] font-bold uppercase tracking-[0.16em] text-brand-300">
              Ein Service von EuroIPL
            </p>
            <div className="mt-6 flex gap-2">
              {[
                { label: "Instagram", href: externalLinks.instagram, icon: "instagram" as const },
                { label: "WhatsApp", href: externalLinks.whatsapp, icon: "whatsapp" as const },
                { label: "Shop", href: externalLinks.shop, icon: "shopping-bag" as const },
              ].map((item) => (
                <a key={item.label} href={item.href} aria-label={item.label} className="group flex h-10 w-10 items-center justify-center rounded-full border border-white/10 text-slate-400 transition hover:-translate-y-1 hover:border-brand-400/50 hover:bg-brand-500 hover:text-white">
                  <Icon name={item.icon} className="h-4 w-4 transition group-hover:scale-110" />
                </a>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2.5">
              ...
              <li>
                <Link href="/dokumente-vorlagen" className="text-sm text-slate-400 hover:text-white">
                  Kostenlose Vorlagen
                </Link>
              </li>
              <li>
                <Link href="/nisv-geraeteanmeldung" className="text-sm text-slate-400 hover:text-white">
