...
export default function PortalServicesPage() {
  return (
    <div>
      <PortalPageHeader
        title="Services"
        description="Derselbe Katalog wie auf geratebuch.de — gerätebezogene Leistungen fordern Sie direkt für ein registriertes Gerät an."
      />
        ...
        {services.map((service) => (
          <Card key={service.slug} className="flex flex-col p-5">
            ...
            <div className="mt-5 flex items-center gap-2">
              <Link
                href={
                  service.requiresDevice === false
                    ? "/kontakt#service-anfragen"
                    : `/portal/serviceanfragen/neu?service=${service.slug}`
                }
                className="flex-1 rounded-btn bg-brand-600 px-3 py-2 text-center text-xs font-semibold text-white hover:bg-brand-700"
              >
                {service.requiresDevice === false ? "Call anfragen" : "Für Gerät anfragen"}
              </Link>
              <Link
